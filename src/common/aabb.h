...
public:
    ...
    double maxMoveOnXclip(const AABB& box, double orgmove) const {
        if (!(intersectY(box) && intersectZ(box))) return orgmove;
        if (min.x >= box.max.x && orgmove < 0.0) return std::max(box.max.x - min.x, orgmove);
        if (max.x <= box.min.x && orgmove > 0.0) return std::min(box.min.x - max.x, orgmove);
        return orgmove;
    }
    ...
    double maxMoveOnYclip(const AABB& box, double orgmove) const {
        if (!(intersectX(box) && intersectZ(box))) return orgmove;
        if (min.y >= box.max.y && orgmove < 0.0) return std::max(box.max.y - min.y, orgmove);
        if (max.y <= box.min.y && orgmove > 0.0) return std::min(box.min.y - max.y, orgmove);
        return orgmove;
    }
    ...
    double maxMoveOnZclip(const AABB& box, double orgmove) const {
        if (!(intersectX(box) && intersectY(box))) return orgmove;
        if (min.z >= box.max.z && orgmove < 0.0) return std::max(box.max.z - min.z, orgmove);
        if (max.z <= box.min.z && orgmove > 0.0) return std::min(box.min.z - max.z, orgmove);
        return orgmove;
    }
    ...
    AABB expand(const Vec3d& arg) const {
        AABB res = *this;
        if (arg.x > 0.0) res.max.x += arg.x; else res.min.x += arg.x;
        if (arg.y > 0.0) res.max.y += arg.y; else res.min.y += arg.y;
        if (arg.z > 0.0) res.max.z += arg.z; else res.min.z += arg.z;
        return res;
    }
};
