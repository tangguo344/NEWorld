...
Logger::Logger(const char* fileName, const char* funcName, int lineNumber, Level level): mLevel(level), mLock(mutex) {
    if (mLevel >= lineLevel) {
        mFileName = fileName;
        mFuncName = funcName;
...
Logger::~Logger() {
    ...
    mContent << std::endl;
    if (!fileOnly) {
        if (mLevel >= cerrLevel) writeOstream(std::cerr);
        else if (mLevel >= coutLevel) writeOstream(std::cout);
    }
    if (mLevel >= fileLevel) {
        for (auto& it : fsink) {
