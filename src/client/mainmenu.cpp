...
MainMenu::MainMenu(UI::Core::Window * win)
{
    ...
    addChild(std::make_shared<UI::Controls::Button>("BTPlay", UI::Core::Margin::CenterCenter(200, 200, -32, 64),
    UI::Locale::Str("play"),[this, win]() { win->pushPage(std::make_shared<GameScene>(win, Application::getApplication().getBlocks(), Application::getApplication().getPlugins()), false, false); }));
    addChild(std::make_shared<UI::Controls::Button>("optionsbtn", UI::Core::Margin::CenterCenter(200, -3, -70, 102),
    UI::Locale::Str("option"), [win]() { win->pushPage(std::make_shared<SettingsMenu>(win), false, false); }));
    addChild(std::make_shared<UI::Controls::Button>("quitbtn", UI::Core::Margin::CenterCenter(-3, 200, -70, 102),
    UI::Locale::Str("exit"), []() { UI::Core::application->terminate(); }));
}
